...
const defaultData = {
  ...
  tables: [
    {id:"T1", name:"مێز 1"}, {id:"T2", name:"مێز 2"}, {id:"T3", name:"مێز 3"},
    {id:"T4", name:"مێز 4"}, {id:"T5", name:"مێز 5"}, {id:"TA", name:"Takeaway"}
  ],
  orders: [],
  sales: []
};
...
let data = loadData();
let state = { page:"pos", cart:[], logged:false, selectedTable:"Takeaway", editingItem:null };
  ...
  <div class="grid two">
    <div class="card">
      ...
      <select id="tableSelect" onchange="state.selectedTable=this.value">
        ${data.tables.map(t=>`<option ${state.selectedTable===t.name?'selected':''}>${t.name}</option>`).join("")}
      </select>
      <div class="categorybar">
        <button class="secondary" onclick="filterMenu('')">هەموو</button>
        ${cats.map(c=>`<button class="secondary" onclick="filterMenu('${c}')">${c}</button>`).join("")}
      </div>
    ...
    <div class="card">
      ...
      <div class="tablewrap">
        <table>
          <thead><tr><th>ئایتم</th><th>دانە</th><th>نرخ</th><th></th></tr></thead>
          <tbody>${state.cart.map((i,idx)=>`
            <tr>
              <td>${i.name}</td>
              <td><input style="width:70px" type="number" min="1" value="${i.qty}" onchange="setQty(${idx},this.value)"></td>
              <td>${money(i.qty*i.price)}</td>
              <td><button class="red" onclick="removeCart(${idx})">X</button></td>
            </tr>`).join("") || `<tr><td colspan="4" class="muted">هیچ شتێک زیاد نەکراوە</td></tr>`}
          </tbody>
        </table>
      </div>
...
function filterMenu(cat){ byId("menuSearch").dataset.cat=cat; renderMenuGrid(); }
function renderMenuGrid(){
  const box=byId("menuGrid"); if(!box) return;
  const search=byId("menuSearch");
  const q=(search.value||"").toLowerCase();
  const cat=search.dataset.cat||"";
  const items=data.menu.filter(x=>(!cat||x.category===cat) && (x.name.toLowerCase().includes(q)||x.code.toLowerCase().includes(q)));
  box.innerHTML=items.map(i=>`
    <button class="itembtn" onclick="addItem('${i.id}')">
      <b>${i.name}</b><span>${i.category}</span><strong>${money(i.price)}</strong>
    </button>`).join("");
}
function addItem(id){
  const p=data.menu.find(x=>x.id===id);
  if(!p) return;
  if(Number(p.stock)<=0) return alert("ئەم ئایتمە نەماوە");
  const item=state.cart.find(x=>x.id===id);
  if(item){ item.qty++; } else state.cart.push({id:p.id,name:p.name,price:Number(p.price),cost:Number(p.cost||0),qty:1});
  render();
}
function setQty(idx,v){ state.cart[idx].qty=Math.max(1,Number(v||1)); render(); }
...
function checkout(){
  ...
  const sale={id:Date.now().toString(), date:new Date().toISOString(), table:state.selectedTable, items:structuredClone(state.cart), subtotal, discount, total, paid, change:paid-total};
...
function saveOpenOrder(){
  if(!state.cart.length) return alert("سەبەتە بەتاڵە");
  data.orders=data.orders.filter(o=>o.table!==state.selectedTable);
  data.orders.unshift({id:Date.now().toString(), table:state.selectedTable, date:new Date().toISOString(), items:structuredClone(state.cart)});
  save();
  alert("Order هەڵگیرا");
  state.cart=[];
  render();
...
function loadOrder(table){
  ...
  if(!o) return alert("هیچ Order ـێک نییە");
...
function printReceipt(sale){
  ...
  <div class="receipt">
    <h3 style="text-align:center">${data.user.shopName}</h3>
    <p>Table: ${sale.table}</p>
    <p>Receipt: ${sale.id}</p>
    <p>Date: ${new Date(sale.date).toLocaleString()}</p><hr>
...
function menuHtml(){
  ...
  <div class="grid two">
    <div class="card">
      ...
      <label>کۆد</label><input id="mCode">
      <label>ناو</label><input id="mName">
      <label>جۆر</label><input id="mCategory" placeholder="Coffee / Tea / Dessert">
      <div class="row">
        <div><label>نرخی فرۆشتن</label><input id="mPrice" type="number"></div>
        <div><label>نرخی کڕین</label><input id="mCost" type="number"></div>
      </div>
      <div class="row">
        <div><label>ستۆک</label><input id="mStock" type="number" value="100"></div>
...
function renderMenuTable(){
  const box=byId("menuTable"); if(!box) return;
  const q=(byId("menuListSearch")?.value||"").toLowerCase();
  const rows=data.menu.filter(p=>p.name.toLowerCase().includes(q)||p.code.toLowerCase().includes(q)).map(p=>`
  <tr>
    <td>${p.code}</td><td>${p.name}</td><td>${p.category}</td><td>${money(p.price)}</td>
    <td><span class="badge ${p.stock<=p.minStock?'low':''}">${p.stock}</span></td>
    <td><button class="blue" onclick="editMenuItem('${p.id}')">Edit</button><button class="red" onclick="deleteMenuItem('${p.id}')">Delete</button></td>
  </tr>`).join("");
...
function saveMenuItem(){
  ...
  if(exists) return alert("ئەم کۆدە پێشتر هەیە");
  const idx=data.menu.findIndex(x=>x.id===item.id);
  if(idx>=0) data.menu[idx]=item; else data.menu.unshift(item);
  state.editingItem=null; save(); render();
}
function editMenuItem(id){
  state.editingItem=id; render();
  const p=data.menu.find(x=>x.id===id);
  byId("mCode").value=p.code; byId("mName").value=p.name; byId("mCategory").value=p.category||"";
  byId("mPrice").value=p.price; byId("mCost").value=p.cost; byId("mStock").value=p.stock; byId("mMin").value=p.minStock||0;
}
...
function tablesHtml(){
  ...
  <div class="grid two">
    <div class="card">
      <h2>زیادکردنی مێز</h2>
      <label>ناوی مێز</label><input id="tableName" placeholder="مێز 6">
      <button class="green" style="margin-top:12px" onclick="addTable()">زیادکردن</button>
    </div>
    <div class="card">
      <h2>Order ـە کراوەکان</h2>
      <div class="tablewrap"><table>
      <thead><tr><th>مێز</th><th>کات</th><th>کۆ</th><th></th></tr></thead>
      <tbody>${data.orders.map(o=>{
        const total=o.items.reduce((s,i)=>s+i.qty*i.price,0);
        return `<tr><td>${o.table}</td><td>${new Date(o.date).toLocaleString()}</td><td>${money(total)}</td><td><button class="blue" onclick="loadOrder('${o.table}')">کردنەوە</button><button class="red" onclick="deleteOrder('${o.table}')">سڕینەوە</button></td></tr>`;
      }).join("")}</tbody>
      </table></div>
    </div>
  </div>
  <div class="card" style="margin-top:14px">
    <h2>مێزەکان</h2>
    <div class="menugrid">${data.tables.map(t=>`<button class="itembtn" onclick="state.selectedTable='${t.name}';state.page='pos';render()"><b>${t.name}</b><span>${data.orders.find(o=>o.table===t.name)?"Order هەیە":"بەتاڵ"}</span></button>`).join("")}</div>
  </div>`;
}
function addTable(){
  const name=byId("tableName").value.trim();
  if(!name) return alert("ناو بنووسە");
  data.tables.push({id:crypto.randomUUID(), name});
  save(); render();
}
...
function reportsHtml(){
  ...
  <div class="card" style="margin-top:14px">
    ...
    <div class="tablewrap"><table>
      <thead><tr><th>ژمارە</th><th>کات</th><th>مێز</th><th>کۆ</th></tr></thead>
      <tbody>${data.sales.slice(0,100).map(s=>`<tr><td>${s.id}</td><td>${new Date(s.date).toLocaleString()}</td><td>${s.table}</td><td>${money(s.total)}</td></tr>`).join("")}</tbody>
    </table></div>
  </div>`;
}
...
function settingsHtml(){
  ...
  <div class="grid two">
    <div class="card">
      ...
      <label>ناوی قاوەخانە</label><input id="shopName" value="${data.user.shopName}">
...
function exportBackup(){
  ...
  const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="cafe-pos-backup.json"; a.click();
}
